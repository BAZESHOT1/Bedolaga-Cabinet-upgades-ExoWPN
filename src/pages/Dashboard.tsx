...
import { useQuery, useMutation, useQueryClient } from '@tanstack/react-query'; // ← ИСПРАВЛЕНО: было '@tanstack/react-react-query'
...
import type { SubscriptionListItem } from '../types'; // ← ДОБАВЛЕНО
...
export default function Dashboard() {
  ...
  // ← ИСПРАВЛЕНО: добавлен тип для s
  const hasActivePaid = (multiSubData?.subscriptions ?? []).some(
    (s: SubscriptionListItem) => !s.is_trial && (s.status === 'active' || s.status === 'limited'),
  );
    ...
    <div className="space-y-6">
      ...
      {isMultiTariff && multiSubData?.subscriptions && multiSubData.subscriptions.length > 0 && (
        <div className="space-y-3">
          ...
          {multiSubData.subscriptions.slice(0, 3).map((sub: SubscriptionListItem) => (  // ← ИСПРАВЛЕНО: добавлен тип
